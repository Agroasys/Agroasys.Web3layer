...
SOURCE_DB="${POSTGRES_SMOKE_SOURCE_DB:-agroasys_smoke_source}"
TARGET_DB="${POSTGRES_SMOKE_TARGET_DB:-agroasys_smoke_target}"
...
docker exec "$SRC_CONTAINER" pg_dump --clean --if-exists --no-owner --no-privileges -U "$POSTGRES_USER" "$SOURCE_DB" > "$DUMP_FILE"
if [[ ! -s "$DUMP_FILE" ]]; then
